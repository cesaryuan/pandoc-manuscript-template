...
If `latex2wmf` exits with a nonzero status for a formula, the build warns with the
...
conversion continues with the `set-data` result. Failed previews are not cached.
