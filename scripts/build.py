...
class BuildSettings(BaseSettings):
    ...
    mathtype_marker_filter: str = "scripts/mathtype/mathtype_markers.lua"
...
def resource_path(path: str | Path) -> Path:
    ...
    if path.is_absolute():
        return path
    return Path(SETTINGS.resource_root) / path
...
def run_mathtype_conversion(marked_docx: Path, target_docx: Path) -> None:
    ...
    run_command(
        [
            ...
            str(target_docx),
            '--work-dir',
            str(Path(SETTINGS.mathtype_work_dir) / SETTINGS.project_name),
        ],
        stream_output=True,
    )
