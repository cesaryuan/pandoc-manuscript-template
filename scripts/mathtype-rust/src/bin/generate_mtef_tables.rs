...
fn build_targets() -> Result<Vec<Target>, String> {
    ...
    // MathType's TeX input does not reliably emit CHAR records for the whole
    // blackboard alphabet, so this stays limited to verified writer coverage.
    for ch in ['I'] {
    ...
    match target.selector {
        Selector::FontPos {
            ...
            Category::MathCal | Category::MathBb => records
                ...
                .rev()
                .copied()
                .find(|record| matches!(record.typeface, 0x7e | 0x7f | 0x83 | 0x88))
                .ok_or_else(|| {
                    format!(
...
fn collect_char_records(mtef: &[u8]) -> Vec<CharRecord> {
    ...
    for index in 0..mtef.len().saturating_sub(4) {
        ...
        if !matches!(typeface, 0x7e | 0x7f | 0x83 | 0x84 | 0x85 | 0x86 | 0x88) {
            continue;
        }
...
fn render_encoded_table(
    ...
    {
        if let Selector::FontPos { ch, .. } = target.selector {
            output.push_str(&format!(
